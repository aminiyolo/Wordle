...
export default function Keypad({ keyword, handleKeypadClick }: KeypadProps) {
  ...
  const color = colorObj[keypadCheck[keyword]];
  return (
    <button
      onClick={() => handleKeypadClick(keyword)}
      className={clsx(
        `${color} bg-[#808384] h-[3rem] w-[2rem] text-center text-2xl !leading-[2.5rem] mx-1 my-1 rounded-md sm:w-11 sm:h-11 sm:!leading-[2.5rem] outline-none `,
